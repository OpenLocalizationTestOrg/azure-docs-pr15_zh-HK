...
本节的目的是 Azure AD 单一登录在 Azure 的传统门户网站中启用并配置单一登录，AppBlade 应用程序中。
...
没有为您在本部分中的操作项。 在试图访问 AppBlade，如果它尚不存在，将创建新用户。 [Azure 的广告单登录配置](#configuring-azure-ad-single-single-sign-on)。
