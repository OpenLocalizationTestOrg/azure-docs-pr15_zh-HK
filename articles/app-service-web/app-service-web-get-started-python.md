<properties 
    pageTitle="在五分钟后将第一个 Python web 应用程序部署到 Azure |Microsoft Azure" 
...
# <a name="deploy-your-first-python-web-app-to-azure-in-five-minutes"></a>在五分钟后将第一个 Python web 应用程序部署到 Azure

本教程将帮助您将第一个 Python web 应用程序部署到[Azure 应用程序服务](../app-service/app-service-value-prop-what-is.md)。
...
## <a name="deploy-a-python-web-app"></a>Python 的 web 应用程序部署
...
4. 唯一的应用程序名称和部署用户先前配置在 Azure 中创建应用程序服务的应用程序资源。 当出现提示时，指定的所需区域。
    ...
    ![在 Azure 创建 Azure 的第一个 web 应用程序资源](./media/app-service-web-get-started-languages/python-site-create.png)
...
4. 部署示例代码于 Azure 应用程序，如将强制使用 Git 的任何代码。 出现提示时，请使用前面配置的密码。
    ...
    ![推送到 Azure 中第一个 web 应用程序的代码](./media/app-service-web-get-started-languages/python-git-push.png)

    `git push`将代码放在 Azure，不仅还会触发部署引擎中的部署任务。 
    如果您有任何 requirements.txt (Python) 文件 （存储库） 项目根目录中，则部署脚本为您恢复所需的程序包。 
...
[创建、 配置和部署到 Visual Studio 中的 Azure Django web 应用程序](web-sites-python-ptvs-django-mysql.md)。 通过遵循本教程，您将了解您需要运行在 Azure，Python 的 web 应用程序的基本技能包括︰

- 创建并部署一个 Python 应用程序，使用模板。
- Python 版本的设置。
- 创建虚拟环境。
- 连接到数据库。
