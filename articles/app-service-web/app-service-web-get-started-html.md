...
本教程将帮助您将一个简单的 HTML + CSS web 应用程序部署到[Azure 应用程序服务](../app-service/app-service-value-prop-what-is.md)。
...
- 部署到它的 HTML 和 CSS。
- 请参阅网页实时在生产环境中运行。
- 更新您的内容相同的方式[推 Git 提交](https://git-scm.com/docs/git-push)。
...
## <a name="deploy-a-simple-html-site"></a>部署一个简单的 HTML 站点
...
4. 部署示例代码于 Azure 应用程序，如将强制使用 Git 的任何代码。 出现提示时，请使用前面配置的密码。
    ...
    如果您使用的语言框架之一，您将看到不同的输出。 这是因为`git push`不仅将代码放在 Azure，但也会触发部署引擎中的部署任务。 如果您有任何 package.json (Node.js) 或 requirements.txt (Python) 文件在您的项目 （存储库） 根或 ASP.NET 项目中有一个 packages.config 文件，则部署脚本将为您恢复所需的程序包。 您还可以[启用作曲者扩展](web-sites-php-mysql-deploy-use-git.md#composer)可以自动处理您的 PHP 应用程序中的 composer.json 文件。
